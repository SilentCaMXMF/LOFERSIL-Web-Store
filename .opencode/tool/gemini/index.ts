...
import { join, dirname, basename, extname, resolve } from "https://deno.land/std/path/mod.ts"
import { getApiKey } from "../env"

// Function to detect if we're in test mode
function isTestMode(): boolean {
  // Only enable test mode when explicitly set
  return Deno.env.get("GEMINI_TEST_MODE") === 'true'
}
...
async function parseImageInput(input: string) {
  ...
  // Treat as file path
  const arr = await Deno.readFile(input)
  const base64 = btoa(String.fromCharCode(...new Uint8Array(arr)))
  // Best-effort mime
  const mime = "image/png" // Default, as Deno doesn't infer MIME easily
  return { mime, base64 }
}
...
function getDateBasedPath(baseDir?: string): string {
  ...
  if (!baseDir) {
    ...
    baseDir = resolve(Deno.cwd(), "../../assets/images")
  }
...
async function getUniqueFilename(directory: string, baseName: string, extension: string, isEdit: boolean = false): Promise<string> {
  ...
  if (!isEdit) {
    ...
    const fileExists = await Deno.stat(baseFilename).then(() => true).catch(() => false)
  ...
  do {
    ...
    counter++
  } while (await Deno.stat(filename).then(() => true).catch(() => false))
...
export async function generateImage(prompt: string, config: ImageConfig = {}): Promise<string> {
  ...
  await Deno.writeFile(outputPath, new Uint8Array(atob(b64).split('').map(c => c.charCodeAt(0))))
